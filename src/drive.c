...
int ata_write_sector0(uint16_t io_base, uint8_t drive, const char* buf)
{
	// 1) Wait for any previous cmd to finish
	if (ata_wait(io_base))
		return -1;

	// 2) Select drive and top LBA bits:
	// 	0xE0 = 1010 0000b -> enable LBA, master by default
